...
Class ImageGenerationTest extends Transition
{
    /**
    ...
    */
    ...
    public function __construct(int $projectId, TuringHead $head, Collection $models)
    {
         ...
         */
        $this->setAgentById(Config::TRANSITION_DALLE_AGENT);
    }
        ...
        $mock->expects(function ($request) {
            $body = (string) $request->getBody();
                    if(json_validate($body))
                    {
                        ...
                        && $requestData->prompt == "A colorful image of AI consciousness emerging."
    ...
    {
        $this->models->get("TestModel")->imageFile = json_decode($data)[0]->fileName;
        $this->models->get("TestModel")->save();
        return $data;
    }
}